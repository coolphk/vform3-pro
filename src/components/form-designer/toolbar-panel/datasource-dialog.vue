...
function onRowExpanded(row) {
  if (row.expanded) {
    ...
    const child = findChild(tableData, row.rowKey)
    // console.log(111, child);

    child['children'] = getDataListByPid(row.rowKey)
  }
}

function onExpandedRowsChange(expandedKeys) {
  // console.log(expandedKeys);
}

function findChild(datas, key) {
  let child = datas.find((data) => data.id === key)
  if (child === undefined) {
    datas.map(data => {
      if (data.children && JSON.stringify(data.children[0]) !== '{}') {
        child = findChild(data.children, key)
      }
    })
  }
  return child
